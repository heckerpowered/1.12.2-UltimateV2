...
public class UltimateClassLoader extends URLClassLoader {

    public static final Proxy ULTIMATE_TRANSFORMER = new Proxy();
    ...
    public UltimateClassLoader(URL[] sources) {
        super(sources, null);
    ...
    public Class<?> findClass(final String name) throws ClassNotFoundException {
        ...
        try {
            ...
            if (lastDot > -1 && !untransformedName.startsWith("net.minecraft.")) {
                if (urlConnection instanceof JarURLConnection) {
                    ...
                    if (pkg == null) {
                        ...
                        LogWrapper.severe("The URL %s is defining elements for sealed path %s", urlConnection.getURL(),
                                packageName);
                    }
                }
            }

            final byte[] transformedClass = runTransformers(untransformedName, transformedName,
                    getClassBytes(untransformedName));
            if (DEBUG_SAVE) {
            ...
            final Class<?> clazz = defineClass(transformedName, transformedClass, 0, transformedClass.length,
                    codeSource);
            ...
            if (DEBUG) {
                ...
                LogManager.getLogger("LaunchWrapper").log(Level.ERROR,
    ...
    private void saveTransformedClass(final byte[] data, final String transformedName) {
        ...
        try {
            LogWrapper.fine("Saving transformed class \"%s\" to \"%s\"", transformedName,
    ...
    private byte[] runTransformers(final String name, final String transformedName, byte[] basicClass) {
        if (DEBUG_FINER) {
            LogWrapper.finest("Beginning transform of {%s (%s)} Start Length: %d", name, transformedName,
                    (basicClass == null ? 0 : basicClass.length));
            for (final IClassTransformer transformer : transformers) {
                final String transName = transformer.getClass().getName();
                LogWrapper.finest("Before Transformer {%s (%s)} %s: %d", name, transformedName, transName,
                        (basicClass == null ? 0 : basicClass.length));
                basicClass = transformer.transform(name, transformedName, basicClass);
                LogWrapper.finest("After  Transformer {%s (%s)} %s: %d", name, transformedName, transName,
                        (basicClass == null ? 0 : basicClass.length));
            }
            LogWrapper.finest("Ending transform of {%s (%s)} Start Length: %d", name, transformedName,
                    (basicClass == null ? 0 : basicClass.length));
        } else {
            for (final IClassTransformer transformer : transformers) {
                basicClass = transformer.transform(name, transformedName, basicClass);
        ...
        return ULTIMATE_TRANSFORMER.transform(name, transformedName, basicClass);
    }
